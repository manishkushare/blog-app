...
class NewPost extends React.Component {
  ...
  handleChange = (event) => {
    ...
    switch (name) {
      case "title":
        errors.title = (this.validatefields(value)) ? "" : "title cannot be empty";
        break;
      case "description":
        errors.description = (this.validatefields(value)) ? "" : "Description cannot be empty";
        break;
      case "body":
        errors.body = (this.validatefields(value)) ? "" : "Body cannot be empty";
        break;
      case "tagList":
        errors.tags = (this.validatefields(value)) ? "" : "tag list cannot be empty";
        break;
    }
    if (name === "tagList") {
      value = value.split(',');
  ...
  handleDisable = () => {
    return (this.state.title && this.state.description && this.state.body && this.state.tagList && !this.state.errors.title && !this.state.errors.body && !this.state.errors.description && !this.state.errors.tags) ? false : true;
  }
  handleSubmit = async (event) => {
    ...
    try {
      let article = await fetch(ARTICLES_URL, {
        method: "POST",
        headers: {
        ...
        <div className="container flex justify-center  ">
          ...
          >
            ...
            <div className="flex justify-end">
              <button type="submit" disabled={this.handleDisable()}>Publish Article</button>
            </div>
          </form>
        </div>
      </section>
